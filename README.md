...
`v1.0.4` 会在 Loon 本地用已保存的原请求做签名公式自检：除旧版规则外，还会测试从 Android 6.03.1 APK 静态分析得到的 `abdf`、签名算法/版本和包名相关组合。脚本只输出“匹配/未匹配”、字段长度和字符类型，不输出 Cookie、Token、原签名、时间戳、设备标识或 nonce 的实际值。自检命中前不会用猜测公式生成新签名。
...
- 当前请求是否匹配已知旧版 SHA-256 拼接规则，以及未匹配时的脱敏结构信息。
